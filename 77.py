...
def extract_metadata(text):
    if not isinstance(text, str) or text.startswith(("HATA", "UYARI")):
        return {"mahkeme": "-", "esas": "-", "karar": "-", "tarih": "-", "davaci": "", "davali": ""}
    ...
    # Davaci ve Davali bilgilerini cikart
    davaci = ""
    davali = ""
    davaci_match = re.search(r"(?i)DAVACI\s*[:\-]?\s*([^\n]{3,50})", text)
    davali_match = re.search(r"(?i)DAVALI\s*[:\-]?\s*([^\n]{3,50})", text)
    if davaci_match:
        davaci = davaci_match.group(1).strip()
    if davali_match:
        davali = davali_match.group(1).strip()
...
def main():
    st.markdown("""
        <div style="text-align: center; padding: 1rem 0 2rem 0;">
            <h1 style="font-size: 2.5rem; margin-bottom: 0.5rem; background: linear-gradient(90deg, #1e3c72, #2a5298); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Hukuk Asistani AI</h1>
            <p style="color: #666; font-size: 1.1rem;">Yapay Zeka Destekli Hukuki Analiz Platformu</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        st.markdown("""
            <div style="text-align: center; padding: 1.5rem 0; margin-bottom: 1rem;">
                <h2 style="color: #fff; margin: 0; font-size: 1.5rem;">Hukuk Asistani AI</h2>
                <p style="color: #a0a0a0; font-size: 0.85rem; margin-top: 0.5rem;">Yapay Zeka Destekli Analiz</p>
            </div>
        """, unsafe_allow_html=True)
        
        st.header("Ayarlar")
        api_key = st.text_input("Google Gemini API Key", type="password", 
                                help="API anahtarinizi Google AI Studio'dan alabilirsiniz")
        
        if api_key:
            st.success("API Baglandi")
        else:
            st.info("API Anahtari Gerekli")
        
        st.caption(f"Kutuphane Surumu: {lib_ver}")
        
        st.divider()
        
        st.markdown("""
            <div style="padding: 1rem; background: rgba(255,255,255,0.05); border-radius: 10px; margin-top: 1rem;">
                <p style="color: #a0a0a0; font-size: 0.8rem; text-align: center; margin: 0;">
                    v10.0 - Modern Edition
                </p>
            </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("Ekrani Temizle", use_container_width=True):
    ...
    # Dava bilgileri belgeden otomatik cikartilir
    # extract_metadata: mahkeme, esas, karar, tarih, davaci, davali dondurur
    input_davaci = auto_data.get("davaci", "")
    input_davali = auto_data.get("davali", "")
    
    _mahkeme_raw = auto_data.get("mahkeme", "")
    input_mahkeme = _mahkeme_raw if _mahkeme_raw not in ["Tespit Edilemedi", "-", ""] else ""
    
    _esas_raw = auto_data.get("esas", "")
    input_dosya_no = _esas_raw if _esas_raw not in ["Bulunamadı", "-", ""] else ""
